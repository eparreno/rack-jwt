  ...
  describe 'handles the exception' do
    ...
    describe '::JWT::VerificationError' do
      ...
      it 'returns a 401 and the correct error msg' do
        ...
        expect(body).to eq(error: {message: 'Invalid JWT token : Signature Verification Error', status: 401})
      end
    end
    ...
    describe '::JWT::ExpiredSignature' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Expired Signature (exp)', status: 401})
      end
    end
    ...
    describe '::JWT::IncorrectAlgorithm' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Incorrect Key Algorithm', status: 401})
      end
    end
    ...
    describe '::JWT::ImmatureSignature' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Immature Signature (nbf)', status: 401})
      end
    end
    ...
    describe '::JWT::InvalidIssuerError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Invalid Issuer (iss)', status: 401})
      end
    end
    ...
    describe '::JWT::InvalidIatError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Invalid Issued At (iat)', status: 401})
      end
    end
    ...
    describe '::JWT::InvalidAudError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Invalid Audience (aud)', status: 401})
      end
    end
    ...
    describe '::JWT::InvalidSubError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Invalid Subject (sub)', status: 401})
      end
    end
    ...
    describe '::JWT::InvalidJtiError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Invalid JWT ID (jti)', status: 401})
      end
    end
    ...
    describe '::JWT::DecodeError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(error: {message: 'Invalid JWT token : Decode Error', status: 401})
      end
    end
  end
end
